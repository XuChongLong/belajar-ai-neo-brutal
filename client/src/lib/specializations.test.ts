...
describe("expanded specialization catalogue", () => {
  it("adds intensive Cloud plus the AI Engineering and Cyber Security curricula", () => {
    expect(materials).toHaveLength(1092);
    expect(new Set(materials.map((material) => material.id)).size).toBe(1092);
    ...
    expect(materials.filter((material) => material.specialization === "automation")).toHaveLength(156);
    expect(materials.filter((material) => material.specialization === "creative-ai")).toHaveLength(156);
    Object.values(specializationMeta).filter((track) => !["ai-engineering", "ai-security", "cloud-devops", "data-engineering", "ai-product", "automation", "creative-ai"].includes(track.id)).forEach((track) => expect(materials.filter((material) => material.specialization === track.id)).toHaveLength(8));
  });
  ...
  it("accepts only known specialization routes and filters their own materials", () => {
    ...
    expect(materials.filter((material) => materialMatchesSpecialization(material, "automation"))).toHaveLength(156);
    expect(materials.filter((material) => materialMatchesSpecialization(material, "creative-ai"))).toHaveLength(156);
    expect(materials.filter((material) => materialMatchesSpecialization(material, "ai-security"))).toHaveLength(252);
    expect(materials.filter((material) => materialMatchesSpecialization(material, null))).toHaveLength(1092);
