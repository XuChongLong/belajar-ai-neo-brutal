...
export default function Materials() {
  ...
  return <div className="page"><div className={`page-wrap catalog-page ${selectedTrack ? `catalog-track-${selectedTrack}` : ""}`}><div className="page-heading"><div><span className="eyebrow">{selectedTrackMeta ? `JALUR ${selectedTrackMeta.label.toUpperCase()} · ${trackMaterials.length} MATERI` : `KATALOG BELAJAR · ${materials.length} MATERI`}</span><h1>{selectedTrackMeta ? <>{selectedTrackMeta.label},<br /><em>mulai dari sini.</em></> : <>Semua materi,<br /><em>pilih sesukamu.</em></>}</h1></div><div className="heading-note"><span className="scribble-arrow">↗</span><p>{selectedTrackMeta ? selectedTrackMeta.intro : <>Urutannya fleksibel.<br />Rasa penasarannya wajib.</>}</p></div></div>
    <section className="progress-tracker"><div className="tracker-copy"><span className="section-index">PROGRESS TRACKER</span><strong>{selectedTrackMeta ? `${trackCompletedCount} dari ${trackMaterials.length} materi di jalur ini selesai` : `${completedCount} dari ${materials.length} materi selesai`}</strong></div><div className="tracker-bar"><i style={{ width: `${selectedTrackMeta ? (trackMaterials.length ? Math.round((trackCompletedCount / trackMaterials.length) * 100) : 0) : progressPercent}%` }} /></div><b>{selectedTrackMeta ? (trackMaterials.length ? Math.round((trackCompletedCount / trackMaterials.length) * 100) : 0) : progressPercent}%</b></section>
    ...
    <section className="catalog-toolbar"><div className="search-box"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={selectedTrackMeta ? `Cari di ${selectedTrackMeta.shortLabel}...` : "Cari materi, misalnya: RAG..."} aria-label="Cari materi" /></div><div className="filter-label"><SlidersHorizontal size={16} /> FILTER</div><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter kategori">{categories.map((item) => <option key={item}>{item}</option>)}</select><select value={level} onChange={(event) => setLevel(event.target.value)} aria-label="Filter level">{levels.map((item) => <option key={item}>{item}</option>)}</select><button type="button" className={`saved-filter ${onlySaved ? "saved-filter-active" : ""}`} onClick={() => setOnlySaved((value) => !value)} aria-pressed={onlySaved}><Bookmark size={15} fill={onlySaved ? "currentColor" : "none"} /> Disimpan <b>{bookmarks.length}</b></button></section>
