...
export default function Glossary() {
  ...
  return <div className="page"><div className="page-wrap glossary-page"><div className="page-heading"><div><span className="eyebrow">GLOSARIUM · MARGIN NOTES</span><h1>Istilah AI,<br /><em>nggak perlu bikin ciut.</em></h1></div><div className="heading-note"><span className="scribble-arrow">↗</span><p>Temukan istilah.<br />Buka konteksnya.</p></div></div>
    <section className="glossary-hero"><div><span className="eyebrow">CATATAN PENDAMPING AI ENGINEERING</span><h2>Istilah teknis, dibikin lebih akrab.</h2><p>Glosarium ini mengikuti materi AI Engineering dari PDF acuan. Mulai dari kata yang bikin kamu berhenti, beb, lalu lanjut ke materi terkait saat kamu siap.</p></div><div className="glossary-count"><strong>{filtered.length}</strong><span>catatan<br />terlihat</span></div></section>
    <div className="glossary-search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari istilah: RAG, evaluasi, token..." aria-label="Cari istilah AI" /></div>
    ...
    {!filtered.length && <div className="empty-state"><span>⊙</span><h2>Istilahnya belum ketemu.</h2><p>Coba kata yang lebih pendek, misalnya RAG, prompt, evaluasi, atau feedback.</p><button className="brutal-button button-black" onClick={() => setSearch("")}>Reset pencarian</button></div>}
  </div></div>;
}
