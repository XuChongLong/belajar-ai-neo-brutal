...
import { ArrowUpRight, Bookmark, BookMarked, BookOpen, ChevronDown, FilePenLine, Flame, FolderKanban, FolderUp, Gamepad2, Home, Info, LayoutDashboard, Layers3, Menu, RotateCcw, Search, Settings2, UserRound, X } from "lucide-react";
...
import { usesReadingFocusShell } from "@/lib/readingFocus";
...
const navGroups = [
  { label: "BELAJAR", items: [{ href: "/", label: "Beranda", icon: Home }, { href: "/materi", label: "Materi & jurusan", icon: BookOpen }, { href: "/tersimpan", label: "Simpan materi", icon: Bookmark }, { href: "/progress", label: "Progress", icon: LayoutDashboard }, { href: "/review", label: "Mode Review", icon: RotateCcw }, { href: "/flashcards", label: "Flashcards", icon: Layers3 }, { href: "/glosarium", label: "Glosarium", icon: BookMarked }] },
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const focusLearningMode = usesReadingFocusShell(location);
  ...
  return (
    <div className={`app-shell ${desktopSidebarOpen ? "desktop-sidebar-open" : ""} ${mobileOpen ? "mobile-nav-open" : ""} ${focusLearningMode ? "app-shell-focus-learning" : ""}`}>
      {!focusLearningMode && <><div className="cursor-dot" style={{ left: cursor.x, top: cursor.y }} aria-hidden="true" />
      ...
      <div className="main-column">
        ...
        <main>{!focusLearningMode && <PageAtmosphereCanvas path={location} />}{children}{!focusLearningMode && <NpcPetPopup />}</main>
      </div>
    </div>
  );
}
